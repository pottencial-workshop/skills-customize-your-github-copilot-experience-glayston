...
As mensagens de commit devem seguir o padrão [Conventional Commits](https://www.conventionalcommits.org/pt/v1.0.0/), garantindo clareza e padronização no histórico do projeto. Exemplos de mensagens:

- `feat: adiciona nova tarefa de análise de dados`
- `fix: corrige erro na exibição das datas de entrega`
- `docs: atualiza documentação das tarefas`
- `refactor: melhora estrutura dos arquivos de configuração`